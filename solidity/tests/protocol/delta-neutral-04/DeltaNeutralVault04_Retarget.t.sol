...
contract DeltaNeutralVault04_RetargetTest is DeltaNeutralVault04Base_Test {
  function setUp() public override {
    super.setUp();
  }

  function testCorrectness_RetargetShouldWorkIfEquityIsNotLost() external {
    _stableVault.setDebt(60 ether, 60 ether);
    _retargetExecutor.setExecutionValue(90 ether, 180 ether);

    _deltaNeutralVault.retarget(abi.encode(0));
  }

  function testRevert_RetargetShouldRevertIfEquityIsLost() external {
    _stableVault.setDebt(100 ether, 100 ether);

    _retargetExecutor.setExecutionValue(500 ether, 400 ether);
