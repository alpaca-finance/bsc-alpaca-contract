...
contract BinancePriceOracle is OwnableUpgradeable, IPriceOracle {
  /// ---------------------------------------------------
  /// Errors
  /// ---------------------------------------------------
  error BinancePriceOracle_InconsistentLength();
  error BinancePriceOracle_InvalidPrice();
  error BinancePriceOracle_NoSource();
  error BinancePriceOracle_SourceExistedPair();
  error BinancePriceOracle_SourceOverLimit();
  ...
  function _setPriceFeed(
    ...
    // Check
    if (priceFeedCount[token1][token0] > 0) revert BinancePriceOracle_SourceExistedPair();
    if (sources.length > 2) revert BinancePriceOracle_SourceOverLimit();
  ...
  function getPrice(address token0, address token1) external view override returns (uint256, uint256) {
    if (priceFeedCount[token0][token1] == 0 && priceFeedCount[token1][token0] == 0)
      revert BinancePriceOracle_NoSource();
