...
contract SpookyWorker03Migrate is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker03 {
  /// @notice Libraries
  ...
  function shareToBalance(uint256 share) public view returns (uint256) {
    ...
    (uint256 totalBalance, ) = activeMasterChef().userInfo(pid, address(this));
  ...
  function migrateLP(ISpookyMasterChefV2 _spookyMasterChefV2, uint256 _newPid) external {
    ...
    address(lpToken).safeApprove(address(_spookyMasterChefV2), uint256(-1));
