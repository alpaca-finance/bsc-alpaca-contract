...
/// @title BiswapWorker03DisabledLiquidation is a worker for force closing positions on low liquidity pools to prevent bad debt from price impact.
contract BiswapWorker03DisabledLiquidation is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker03 {
  /// @notice Libraries
  ...
  function getMktSellAmount(uint256 aIn, uint256 rIn, uint256 rOut, uint256 fee) public view returns (uint256) {
    if (aIn == 0) return 0;
