...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  */
  ...
  await timelock.queueTransaction(
    ...
    ethers.utils.defaultAbiCoder.encode(["uint256", "address", "bool"], [ALLOC_POINT, debtToken.address, true]),
    EXACT_ETA
  );
  console.log("✅ Done");
  ...
  console.log(
    `await timelock.executeTransaction('${FAIR_LAUNCH}', '0', 'addPool(uint256,address,bool)', ethers.utils.defaultAbiCoder.encode(['uint256','address','bool'], [${ALLOC_POINT}, '${debtToken.address}', true]), ${EXACT_ETA})`
  );
  console.log("✅ Done");
};
