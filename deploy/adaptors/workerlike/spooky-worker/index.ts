...
import { SpookyWorker03, SpookyWorker03__factory } from "../../../../typechain";
...
export class SpookyWorkerAdaptor implements IWorkerLike {
  private _worker: SpookyWorker03;
  ...
  constructor(
    ...
    this._worker = SpookyWorker03__factory.connect(_workerAddress, _signerOrProvider);
  ...
  public async validateConfig(
    ...
    try {
      ...
      >([
        ...
        { contract: this._worker, functionName: "spookyMasterChef" },
      ...
      expect(workerMasterChef).to.be.eq(workerInfo.stakingTokenAt, "spookyMasterChef mis-config");
      expect(workerRouter).to.be.eq(routerAddress, "router mis-config");
      expect(workerFee).to.be.eq("998");
