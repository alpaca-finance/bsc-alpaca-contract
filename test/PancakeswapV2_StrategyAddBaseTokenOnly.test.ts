...
describe('PancakeswapV2 - StrategyAddBaseTokenOnly', () => {
  ...
  it('should convert all BTOKEN to LP tokens at best rate', async () => {
    ...
    expect(await lpV2.balanceOf(await bob.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('0.015415183776793567'))
    ...
    await baseTokenAsBob.transfer(strat.address, ethers.utils.parseEther('0.1'));
    await lpAsBob.transfer(strat.address, ethers.utils.parseEther('0.015415183776793567'));
    await stratAsBob.execute(
      await bob.getAddress(), '0',
      ethers.utils.defaultAbiCoder.encode(
    ...
    expect(await lpV2.balanceOf(await bob.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('0.030143537666997650'))
    expect(await lpV2.balanceOf(strat.address)).to.be.bignumber.eq(ethers.utils.parseEther('0'))
    expect(await farmingToken.balanceOf(strat.address)).to.be.bignumber.eq(ethers.utils.parseEther('0'))
