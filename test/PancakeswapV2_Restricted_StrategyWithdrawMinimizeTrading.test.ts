...
describe('PancakeswapV2RestrictedStrategyWithdrawMinimizeTrading', () => {
  ...
  beforeEach(async () => {
    ...
    strat = await upgrades.deployProxy(PancakeswapV2RestrictedStrategyWithdrawMinimizeTrading, [routerV2.address, wbnb.address, wNativeRelayer.address]) as PancakeswapV2RestrictedStrategyWithdrawMinimizeTrading;
  ...
  context("When caller worker hasn't been whitelisted", async () => {
    it('should revert as bad worker', async () => {
      ...
      )).to.be.revertedWith('PancakeswapV2RestrictedStrategyWithdrawMinimizeTrading::onlyWhitelistedWorkers:: bad worker');
    });
  })
  ...
  context('It should convert LP tokens and farming token', () => {
    ...
    it('should revert, Bob uses withdraw minimize trading strategy to turn LPs back to farming with an unreasonable expectation', async () => {
      ...
      )).to.be.revertedWith('PancakeswapV2RestrictedStrategyWithdrawMinimizeTrading::execute:: insufficient farming tokens received')
    });
