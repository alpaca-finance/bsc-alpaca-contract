...
// initial Contract Signer part
  ...
  await mockMdexWorker.deployed();
    ...
    swapMining = await SwapMining.deploy(mdxToken.address,factory.address,oracle.address, router.address,farmingToken.address, MDX_PER_BLOCK,blockNumber)
    ...
    await mdxToken["addMinter(address)"](swapMining.address)
    ...
    strat = (await upgrades.deployProxy(MdexRestrictedStrategyLiquidate, [router.address,mdxToken.address])) as 
    ...
    context("When apply liquite strategy", async ()=> {
        it("should convert all LP token back to ", async() => {
          ...
          // assert mining
          expect(mdexTokenAfter.sub(mdexTokenBefore)).to.gt(0)
        })
    })
