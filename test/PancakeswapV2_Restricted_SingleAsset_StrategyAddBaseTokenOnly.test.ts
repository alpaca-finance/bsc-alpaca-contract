...
describe('PancakeswapV2RestrictedSingleAssetStrategyAddBaseTokenOnly', () => {
  ...
  beforeEach(async () => {
    ...
    mockPancakeswapV2EvilWorker = await MockPancakeswapV2CakeMaxiWorker.deploy(
      ...
      [farmingToken.address, wbnb.address]) as MockPancakeswapV2CakeMaxiWorker
    ...
    strat = await upgrades.deployProxy(PancakeswapV2RestrictedSingleAssetStrategyAddBaseTokenOnly, [routerV2.address]) as PancakeswapV2RestrictedSingleAssetStrategyAddBaseTokenOnly;
  ...
  context('when the base token is a wrap native', async () => {
    context('When contract get farmingToken amount < minFarmingTokenAmount', async () => {
        it('should revert', async () => {
          ...
          )).to.be.revertedWith('PancakeswapV2RestrictedSingleAssetStrategyAddBaseTokenOnly::execute:: insufficient farmingToken amount received');
        });
      })
    ...
    context("When caller worker hasn't been whitelisted", async () => {
      it('should revert as bad worker', async () => {
        ...
        )).to.be.revertedWith('PancakeswapV2RestrictedSingleAssetStrategyAddBaseTokenOnly::onlyWhitelistedWorkers:: bad worker');
      });
    })
