...
import { DeltaNeutralBiswapWorker03 } from "../../../../typechain/DeltaNeutralBiswapWorker03";
import { DeltaNeutralBiswapWorker03__factory } from "../../../../typechain/factories/DeltaNeutralBiswapWorker03__factory";
...
describe("DeltaNeutralVaultGatewayWithBiswap", () => {
  ...
  let stableTwoSidesStrat: BiswapStrategyAddTwoSidesOptimal;
  let assetTwoSidesStrat: BiswapStrategyAddTwoSidesOptimal;
  ...
  let stableVaultWorker: DeltaNeutralBiswapWorker03;
  let assetVaultWorker: DeltaNeutralBiswapWorker03;
  ...
  let biswapMasterChefAsAlice: BiswapMasterChef;
  let biswapMasterChefAsBob: BiswapMasterChef;

  let biswapWorker03AsEve: DeltaNeutralBiswapWorker03__factory;
  let biswapWorker0301AsEve: DeltaNeutralBiswapWorker03__factory;
  ...
  async function fixture() {
    ...
    [addStrat, liqStrat, stableTwoSidesStrat, minimizeStrat, partialCloseStrat, partialCloseMinimizeStrat] =
      await deployHelper.deployBiswapStrategies(router, stableVault, wNativeRelayer);

    const BiswapStrategyAddTwoSidesOptimal = (await ethers.getContractFactory(
      "BiswapStrategyAddTwoSidesOptimal",
      deployer
    )) as BiswapStrategyAddTwoSidesOptimal__factory;
    assetTwoSidesStrat = (await upgrades.deployProxy(BiswapStrategyAddTwoSidesOptimal, [
      router.address,
      assetVault.address,
    ...
    lp = BiswapPair__factory.connect(await factory.getPair(wbnb.address, baseToken.address), deployer);
    ...
    stableVaultWorker = await deployHelper.deployDeltaNeutralBiswapWorker03(
      stableVault,
      baseToken,
      masterChef,
      router,
    ...
    /// Setup DeltaNeutralBiswapWorker03
    assetVaultWorker = await deployHelper.deployDeltaNeutralBiswapWorker03(
      assetVault,
      wbnb as unknown as MockERC20,
      masterChef,
      router,
    ...
    biswapMasterChefAsAlice = BiswapMasterChef__factory.connect(masterChef.address, alice);
