...
describe("BiswapStrategyAddTwoSideOptimal", () => {
  ...
  let addRestrictedStrat: BiswapStrategyAddTwoSidesOptimal;
  let addRestrictedStratAsDeployer: BiswapStrategyAddTwoSidesOptimal;

  // Contract Signer
  let addRestrictedStratAsBob: BiswapStrategyAddTwoSidesOptimal;
  ...
  const setupRestrictedTest = async () => {
    ...
    addRestrictedStrat = (await upgrades.deployProxy(BiswapStrategyAddTwoSidesOptimal, [
      router.address,
      mockedVault.address,
  ...
  const setupContractSigner = async () => {
    // Contract signer
    addRestrictedStratAsBob = BiswapStrategyAddTwoSidesOptimal__factory.connect(
      addRestrictedStrat.address,
      bob
    );
    addRestrictedStratAsDeployer = BiswapStrategyAddTwoSidesOptimal__factory.connect(
      addRestrictedStrat.address,
      deployer
    );
  ...
  describe("full flow test", async () => {
    ...
    it("should convert all BTOKEN to LP tokens at best rate", async () => {
      ...
      Assert.assertAlmostEqual(await (await lp.balanceOf(mockMdexWorker.address)).toString(), ethers.utils.parseEther("0.240426434911385908").toString());
