...
describe("SolidlyStrategyPartialCloseMinimizeTrading", () => {
  ...
  /// Strategy instance(s)
  let strat: SolidlyStrategyPartialCloseMinimizeTrading;
  ...
  let stratAsAlice: SolidlyStrategyPartialCloseMinimizeTrading;
  let stratAsBob: SolidlyStrategyPartialCloseMinimizeTrading;
  ...
  async function fixture() {
    ...
    strat = (await upgrades.deployProxy(SolidlyStrategyPartialCloseMinimizeTrading, [
      router.address,
    ...
    stratAsAlice = SolidlyStrategyPartialCloseMinimizeTrading__factory.connect(strat.address, alice);
    stratAsBob = SolidlyStrategyPartialCloseMinimizeTrading__factory.connect(strat.address, bob);
    ...
    context("when maxLpTokenToLiquidate > LP from worker", async () => {
      it("should use all LP", async () => {
        ...
        await expect(
          ...
          .to.emit(strat, "LogSolidlyStrategyPartialCloseMinimizeTrading")
          .withArgs(
            baseToken.address,
            farmingToken.address,
    ...
    context("when some trade (maxReturnDebt > received BTOKEN from LP)", async () => {
      context(
        ...
        async () => {
          ...
          it("should be revert", async () => {
            ...
            ).to.revertedWith("insufficient farming token");
          });
        }
      );
      ...
      context("when farming tokens received >= slippage", async () => {
        ...
        it("should be successfully", async () => {
          ...
          TestHelpers.assertAlmostEqual(
            ethers.utils.parseEther("3.191408502552382874").toString(),
            bobFTOKENAfter.sub(bobFTOKENBefore).toString()
          );
        });
      });
    ...
    context("when some trade (maxReturnDebt > received Base token from LP)", async () => {
      ...
      context("when farming tokens received >= slippage", async () => {
        ...
        it("should be successfully", async () => {
          ...
          TestHelpers.assertAlmostEqual(
            ethers.utils.parseEther("0.039284642749989283").toString(),
            bobBnbAfter.sub(bobBnbBefore).toString()
          );
        });
      });
