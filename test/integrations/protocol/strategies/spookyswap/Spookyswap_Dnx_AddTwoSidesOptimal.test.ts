...
import {
  ...
  SpookySwapDnxStrategyAddTwoSidesOptimal__factory,
} from "../../../../../typechain";
...
describe("SpookyswapDnxStrategyAddTwoSideOptimal", () => {
  ...
  let addTwoSides: SpookySwapDnxStrategyAddTwoSidesOptimal;

  // Contract Signer
  let addTwoSidesAsBob: SpookySwapDnxStrategyAddTwoSidesOptimal;
  ...
  const setupRestrictedTest = async () => {
    ...
    const SpookySwapDnxStrategyAddTwoSidesOptimal = (await ethers.getContractFactory(
      "SpookySwapDnxStrategyAddTwoSidesOptimal",
      deployer
    )) as SpookySwapDnxStrategyAddTwoSidesOptimal__factory;
    addTwoSides = (await upgrades.deployProxy(SpookySwapDnxStrategyAddTwoSidesOptimal, [
      router.address,
      mockedVault.address,
    ])) as SpookySwapDnxStrategyAddTwoSidesOptimal;
  ...
  const setupContractSigner = async () => {
    // Contract signer
    addTwoSidesAsBob = SpookySwapDnxStrategyAddTwoSidesOptimal__factory.connect(addTwoSides.address, bob);
  ...
  describe("full flow test", async () => {
    ...
    it("should convert all BTOKEN to LP tokens at best rate", async () => {
      ...
      expect(await baseToken.balanceOf(addTwoSides.address)).to.be.below(MAX_ROUNDING_ERROR);
    ...
    it("shouldn't convert some BTOKEN to LP tokens at best rate when some dust BTOKEN", async () => {
      ...
      await farmingTokenAsAlice.approve(mockedVault.address, ethers.utils.parseEther("1"));
      // dust
      await baseToken.mint(addTwoSides.address, ethers.utils.parseEther("0.000000000000000005"));
      await mockWorkerAsAlice.work(
        0,
        await alice.getAddress(),
        ethers.utils.parseEther("0"),
    ...
    it("shouldn't convert some FTOKEN to LP tokens at best rate when some dust FTOKEN", async () => {
      ...
      await farmingTokenAsAlice.approve(mockedVault.address, ethers.utils.parseEther("1"));
      // dust
      await farmingToken.mint(addTwoSides.address, ethers.utils.parseEther("0.000000000000000005"));
      await mockWorkerAsAlice.work(
        0,
        await alice.getAddress(),
        ethers.utils.parseEther("0"),
