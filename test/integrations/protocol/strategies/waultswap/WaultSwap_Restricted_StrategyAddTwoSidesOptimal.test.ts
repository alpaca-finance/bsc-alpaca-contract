...
describe("WaultSwapRestrictedStrategyAddTwoSideOptimal", () => {
  ...
  const setupRestrictedTest = async () => {
    ...
    addRestrictedStrat = (await upgrades.deployProxy(WaultSwapRestrictedStrategyAddTwoSidesOptimal, [
      router.address,
      mockedVault.address,
  ...
  const setupContractSigner = async () => {
    // Contract signer
    addRestrictedStratAsBob = WaultSwapRestrictedStrategyAddTwoSidesOptimal__factory.connect(
      addRestrictedStrat.address,
      bob
    );
    addRestrictedStratAsDeployer = WaultSwapRestrictedStrategyAddTwoSidesOptimal__factory.connect(
      addRestrictedStrat.address,
      deployer
    );
  ...
  describe("full flow test", async () => {
    ...
    context("when bad calldata", async () => {
      it("should revert", async () => {
        ...
        await expect(
          mockWaultSwapWorkerAsAlice.work(
            0,
    ...
    it("should convert all BTOKEN to LP tokens at best rate", async () => {
      ...
      await mockWaultSwapWorkerAsAlice.work(
        0,
        await alice.getAddress(),
      ...
      Assert.assertAlmostEqual(
        (await lpV2.balanceOf(mockWaultSwapWorker.address)).toString(),
