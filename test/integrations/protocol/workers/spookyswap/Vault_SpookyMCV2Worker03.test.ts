...
describe("Vault - SpookyMCV2Worker03", () => {
  ...
  let masterChefV2: SpookyMasterChefV2;
  let spookyWorker: SpookyMCV2Worker03;
  ...
  async function fixture() {
    ...
    // Setup MasterChefV2
    // Deploy dummyToken for MasterChefV2
    const dummyToken = await deployHelper.deployERC20();
    await dummyToken.mint(deployerAddress, 1);

    // Add MCV2 Master Pool
    await masterChef.add(1, dummyToken.address);

    // Deploy MasterChefV2
    const SpookyMasterChefV2 = (await ethers.getContractFactory("SpookyMasterChefV2")) as SpookyMasterChefV2__factory;
    masterChefV2 = await SpookyMasterChefV2.deploy(masterChef.address, boo.address, 0);
    await dummyToken.approve(masterChefV2.address, 1);
    await masterChefV2.init(dummyToken.address);

    // Add a pool to MasterChefV2
    await masterChefV2.add(1, lp.address, ethers.constants.AddressZero, true);

    /// Setup SpookyMCV2Worker03
    const SpookyMCV2Worker03 = (await ethers.getContractFactory(
      "SpookyMCV2Worker03",
      deployer
    )) as SpookyMCV2Worker03__factory;
    spookyWorker = (await upgrades.deployProxy(SpookyMCV2Worker03, [
      vault.address,
      baseToken.address,
    ...
    workerHelper = new Worker02Helper(spookyWorker.address, masterChefV2.address);
  ...
  context("when user uses LYF", async () => {
    context("#work", async () => {
      ...
      it("should close position correctly when user holds multiple positions", async () => {
        ...
        // Expect
        let [workerLpAfter] = await masterChefV2.userInfo(POOL_ID, spookyWorker.address);
        ...
        [workerLpBefore] = await masterChefV2.userInfo(POOL_ID, spookyWorker.address);
        ...
        stages["afterReinvest"] = await TimeHelpers.latest();
        [workerLpAfter] = await masterChefV2.userInfo(POOL_ID, spookyWorker.address);
        ...
        let [workerLPBefore] = await masterChefV2.userInfo(POOL_ID, spookyWorker.address);
        ...
        context("when maxReturn is lessDebt", async () => {
          ...
          it("should pay debt 'maxReturn' BTOKEN and return 'liquidatedAmount - maxReturn' BTOKEN to user", async () => {
            ...
            const [path, reinvestPath] = await Promise.all([
              spookyWorker.getPath(),
              spookyWorker["getReinvestPath()"](),
            ]);
    ...
    context("#addCollateral", async () => {
      ...
      async function successBtokenOnly(lastWorkBlockTimestamp: BigNumber, goRouge: boolean) {
        ...
        const [workerLpBefore] = await masterChefV2.userInfo(POOL_ID, spookyWorker.address);
