...
describe("Vault2 - SpookyWorker03", () => {
  ...
  context("when worker is initialized", async () => {
    ...
    it("should initialized the correct fee and feeDenom", async () => {
      expect(await spookyWorker.fee()).to.be.bignumber.eq("998");
      expect(await spookyWorker.feeDenom()).to.be.bignumber.eq("1000");
    });
  });
  ...
  context("when owner is setting worker", async () => {
    describe("#setReinvestConfig", async () => {
      it("should set reinvest config correctly", async () => {
        ...
        expect(await spookyWorker.reinvestBountyBps()).to.be.bignumber.eq(250);
        expect(await spookyWorker.reinvestThreshold()).to.be.bignumber.eq(ethers.utils.parseEther("1"));
      ...
      it("should revert when owner set reinvestBountyBps > max", async () => {
        ...
        expect(await spookyWorker.reinvestBountyBps()).to.be.bignumber.eq(100);
      });
    ...
    describe("#setMaxReinvestBountyBps", async () => {
      it("should set max reinvest bounty", async () => {
        ...
        expect(await spookyWorker.maxReinvestBountyBps()).to.be.bignumber.eq(200);
      });
  ...
  context("when user uses LYF", async () => {
    context("#work", async () => {
      ...
      it("should work", async () => {
        ...
        expect(await spookyWorker.health(1)).to.be.bignumber.eq(ethers.utils.parseEther("1.998307255271658491"));
        ...
        expect(positionInfo[0]).to.be.bignumber.above(ethers.utils.parseEther("2"));
      ...
      it("should close position correctly when user holds multiple positions", async () => {
        ...
        expect(await spookyWorker.shares(1), `expect Pos#1 has ${shares[0]} shares`).to.be.bignumber.eq(shares[0]);
        expect(
          await spookyWorker.shareToBalance(await spookyWorker.shares(1)),
          `expect Pos#1 LPs = ${expectedLp}`
        ).to.be.bignumber.eq(expectedLp);
        expect(await spookyWorker.totalShare(), `expect totalShare = ${totalShare}`).to.be.bignumber.eq(totalShare);
        expect(
        ...
        expect(await spookyWorker.shares(2), `expect Pos#2 has ${shares[1]} shares`).to.be.bignumber.eq(shares[1]);
        expect(
        ...
        expect(eveBooAfter.sub(eveBooBefore), `expect eve's WEX to remain the same`).to.be.bignumber.eq("0");
        expect(workerLpAfter, `expect Worker to stake ${accumLp} LP`).to.be.bignumber.eq(accumLp);
        expect(
          await baseToken.balanceOf(addStrat.address),
          `expect add BTOKEN strat to have ${debrisBtoken} BTOKEN debris`
        ...
        expect(deployerBooAfter.sub(deployerBooBefore), `expect DEPLOYER's WEX to remain the same`).to.be.bignumber.eq(
          "0"
        );
        expect(eveBooAfter.sub(eveBooBefore), `expect eve to get ${reinvestFees}`).to.be.bignumber.eq(reinvestFees);
      ...
      context("#liquidate", async () => {
        ...
        context("when debt is lessDebt", async () => {
          ...
          it("should pay back all debt and return 'liquidatedAmount - debt' BTOKEN to user", async () => {
            ...
            expect(bobAfter.sub(bobBefore), `expect Bob get ${liquidatedBtoken} BTOKEN`).to.be.bignumber.eq(
              liquidatedBtoken
            );
            // Check Bob position info
    ...
    context("#addCollateral", async () => {
      ...
      beforeEach(async () => {
        ...
        expect(await spookyWorker.health(1)).to.be.bignumber.eq(expectedHealth);
      ...
      async function successBtokenOnly(lastWorkBlockTimestamp: BigNumber, goRouge: boolean) {
        ...
        expect(await spookyWorker.shares(1), `expect Alice's shares = ${accumLp}`).to.be.bignumber.eq(accumLp);
        expect(
          await spookyWorker.shareToBalance(await spookyWorker.shares(1)),
      ...
      async function successTwoSides(lastWorkBlockTimestamp: BigNumber, goRouge: boolean) {
        ...
        expect(await boo.balanceOf(DEPLOYER), `expect Deployer gets ${totalReinvestFees} WEX`).to.be.bignumber.eq(
          totalReinvestFees
        );
        expect(
