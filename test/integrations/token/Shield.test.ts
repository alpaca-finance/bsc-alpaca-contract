...
describe("Shield", () => {
  ...
  context("when migrate FairLaunchV1's owner from Timelock to Shield + Timelock", async () => {
    ...
    it("should revert when non owner to interact with Shield", async () => {
      await expect(shield.setAlpacaPerBlock(ethers.utils.parseEther("1"))).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );

      await expect(shield.setBonus(1, 500, 7000)).to.be.revertedWith("Ownable: caller is not the owner");

      await expect(shield.mintWarchest(await dev.getAddress(), ethers.utils.parseEther("1"))).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );

      await expect(shield.addPool(1, stakingTokens[0].address, false)).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );

      await expect(shield.setPool(1, 100, false)).to.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    it("should revert when adjust param through Timelock + Shield when migration has not been done", async () => {
      ...
      await timelockAsDev.queueTransaction(
        ...
        ethers.utils.defaultAbiCoder.encode(["uint256", "address", "bool"], [100, stakingTokens[0].address, false]),
        eta
      );
    ...
    it("should migrate successfully", async () => {
      ...
      await timelockAsDev.queueTransaction(
        ...
        ethers.utils.defaultAbiCoder.encode(["address"], [shield.address]),
        eta
      );
  ...
  context("when adjust FairLaunchV1 params via Shield + Timelock", async () => {
    ...
    it("should set bonus on FLV1 when Timelock is passed ETA", async () => {
      ...
      await timelockAsDev.queueTransaction(
        ...
        ethers.utils.defaultAbiCoder.encode(["uint256", "uint256", "uint256"], [2, 888888, 7000]),
        eta
      );
    ...
    it("should allow to mint Alpaca if mintCount <= 8m", async () => {
      ...
      await timelockAsDev.queueTransaction(
        ...
        ethers.utils.defaultAbiCoder.encode(
          ["address", "uint256"],
          [await alice.getAddress(), ethers.utils.parseEther("2750000")]
        ),
        eta
      );
      ...
      await timelockAsDev.queueTransaction(
        ...
        ethers.utils.defaultAbiCoder.encode(
          ["address", "uint256"],
          [await alice.getAddress(), ethers.utils.parseEther("5000000")]
        ),
        eta
      );
    ...
    it("should revert when mintCount > 8m", async () => {
      ...
      await timelockAsDev.queueTransaction(
        ...
        ethers.utils.defaultAbiCoder.encode(
          ["address", "uint256"],
          [await alice.getAddress(), ethers.utils.parseEther("5000001")]
        ),
        eta
      );
