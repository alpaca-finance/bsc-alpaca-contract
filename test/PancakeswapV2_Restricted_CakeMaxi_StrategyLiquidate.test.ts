...
describe('PancakeswapV2RestrictedCakeMaxiStrategyLiquidate', () => {
  ...
  /// Strategy instance(s)
  let strat: PancakeswapV2RestrictedCakeMaxiStrategyLiquidate;
  ...
  let stratAsAlice: PancakeswapV2RestrictedCakeMaxiStrategyLiquidate;
  let stratAsBob: PancakeswapV2RestrictedCakeMaxiStrategyLiquidate;
  ...
  beforeEach(async () => {
    ...
    stratAsAlice = PancakeswapV2RestrictedCakeMaxiStrategyLiquidate__factory.connect(strat.address, alice);
    stratAsBob = PancakeswapV2RestrictedCakeMaxiStrategyLiquidate__factory.connect(strat.address, bob);
  ...
  context('When the base token is a wrap native', async () => {
    context('When contract get baseToken amount < mingBaseTokenAmount', async () => {
        it('should revert', async () => {
          ...
          )).to.be.revertedWith('PancakeswapV2RestrictedCakeMaxiStrategyLiquidate::execute:: insufficient baseToken amount received');
        });
      })
    ...
    context("When caller worker hasn't been whitelisted", async () => {
      it('should revert as bad worker', async () => {
        ...
        )).to.be.revertedWith('PancakeswapV2RestrictedCakeMaxiStrategyLiquidate::onlyWhitelistedWorkers:: bad worker');
      });
    })
