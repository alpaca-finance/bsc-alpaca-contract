...
describe("WaultSwapRestrictedStrategyPartialCloseMinimizeTrading", () => {
  ...
  beforeEach(async () => {
    ...
    strat = (await upgrades.deployProxy(WaultSwapRestrictedStrategyPartialCloseMinimizeTrading, [
      router.address,
      wbnb.address,
  ...
  context("when non-worker call the strat", async () => {
    it("should revert", async () => {
      ...
      ).to.revertedWith("WaultSwapRestrictedStrategyPartialCloseMinimizeTrading::onlyWhitelistedWorkers:: bad worker");
    });
  });
  ...
  context("when bad calldata", async () => {
    it("should revert", async () => {
      await expect(mockWaultSwapWorkerAsBob.work(0, bobAddress, "0", "0x1234")).to.reverted;
    });
  });
