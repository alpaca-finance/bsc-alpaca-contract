...
describe('CakeMaxiWorker02', () => {
  ...
  beforeEach(async () => {
    ...
    await simpleVaultConfig.setWorker(integratedCakeMaxiWorker01.address, true, true, WORK_FACTOR, KILL_FACTOR, true);
  ...
  describe("#setTreasuryBountyBps", async() => {
    context('when treasury bounty > max reinvest bounty', async () => {
      it('should revert', async() => {
        await expect(cakeMaxiWorkerNative.setTreasuryConfig(DEPLOYER, parseInt(MAX_REINVEST_BOUNTY) + 1)).to.revertedWith('CakeMaxiWorker02::setTreasuryConfig:: _treasuryBountyBps exceeded maxReinvestBountyBps');
  ...
  describe("#reinvest()", async() => {
    context("When the caller is not a reinvestor", async () => {
      it('should be reverted', async () => {
        await expect(cakeMaxiWorkerNativeAsAlice.reinvest()).to.revertedWith('CakeMaxiWorker02::onlyReinvestor:: not reinvestor')
      })
    })
    ...
    context("When integrated with an actual vault", async () => {
      it('should reinvest with updated beneficial vault reward to the beneficial vault', async () => {
        ...
        expect(await integratedCakeMaxiWorker.shareToBalance(await integratedCakeMaxiWorker.shares(1))).to.eq(alicePos1Balance)
        expect(await integratedCakeMaxiWorker.shareToBalance(await integratedCakeMaxiWorker.shares(2))).to.eq(bobPos2Balance)
        Assert.assertAlmostEqual((await wbnb.balanceOf(integratedVault.address)).sub(ethers.utils.parseEther('1').sub(ethers.utils.parseEther('0.05'))).toString(), ethers.utils.parseEther('0.001431176510697250').add(ethers.utils.parseEther('0.001205378386656404')).toString())
        ...
        expect(await integratedCakeMaxiWorker.shareToBalance(await integratedCakeMaxiWorker.shares(1))).to.eq(alicePos1Balance)
        expect(await integratedCakeMaxiWorker.shareToBalance(await integratedCakeMaxiWorker.shares(2))).to.eq(bobPos2Balance)
        expect(await integratedCakeMaxiWorker.shareToBalance(await integratedCakeMaxiWorker.shares(3))).to.eq(alicePos3Balance)
        Assert.assertAlmostEqual((await wbnb.balanceOf(integratedVault.address)).sub(ethers.utils.parseEther('1').sub(ethers.utils.parseEther('0.05'))).toString(), ethers.utils.parseEther('0.001431176510697250').add(ethers.utils.parseEther('0.001205378386656404')).toString())
        Assert.assertAlmostEqual((await integratedCakeMaxiWorker.buybackAmount()).toString(), ethers.utils.parseEther('0.003099782083742242').toString())
        Assert.assertAlmostEqual((await integratedCakeMaxiWorker.rewardBalance()).toString(), ethers.utils.parseEther('0').toString())
      })
      ...
      context("when the worker is an older version", async() => {
        context("with upgrade during the flow", async () => {
          it('should reinvest with updated beneficial vault reward to the beneficial vault', async() => {
            ...
            expect(userInfo[0], "expect worker has 0.016630354291151718 CAKE staked in MasterChef")
              .to.eq(ethers.utils.parseEther('0.016630354291151718'))
            expect(await integratedCakeMaxiWorker02.shares(1), "expect Alice's shares = 0.016630354291151718")
              .to.eq(ethers.utils.parseEther('0.016630354291151718'))
            expect(await cake.balanceOf(DEPLOYER), "expect DEPLOYER's CAKE remains 0 as no reinvest happened")
              .to.be.bignumber.eq(0)
            expect(await integratedCakeMaxiWorker02.rewardBalance(), "expect worker.rewardBalance is 0.199999999999998096")
            ...
            Assert.assertAlmostEqual(
              (await wbnb.balanceOf(integratedVault.address)).sub(ethers.utils.parseEther('1').sub(ethers.utils.parseEther('0.05'))).toString(),
              ethers.utils.parseEther('0').toString())
            expect(await integratedCakeMaxiWorker02.buybackAmount(), "expect buybackAmount is 0 as no reinvest happened")
              .to.be.bignumber.eq(0)
            ...
            userInfo = await masterChef.userInfo(0, integratedCakeMaxiWorker02.address)
            expect(userInfo[0], "expect worker staked 0.313630354291135708 CAKE")
              .to.be.eq(ethers.utils.parseEther('0.313630354291135708'))
            expect(await cake.balanceOf(await eve.getAddress()), "expect Eve's CAKE is 0.002699999999999855")
              .to.be.eq(ethers.utils.parseEther('0.002699999999999855'))
            expect(
              (await wbnb.balanceOf(integratedVault.address)).sub(ethers.utils.parseEther('1').sub(ethers.utils.parseEther('0.05'))),
            ...
            expect(userInfo[0], "expect worker staked 0.518102509290200784 CAKE")
              .to.eq(ethers.utils.parseEther('0.518102509290200784'))
            expect(await integratedCakeMaxiWorker02.shares(2), "expect Bob's share is correct")
              .to.eq(bobShare)
            expect(
              await integratedCakeMaxiWorker02.shareToBalance(await integratedCakeMaxiWorker02.shares(2)),
              "expect Bob's balance is correct").to.eq(bobBalance)
            expect(await integratedCakeMaxiWorker02.shareToBalance(await integratedCakeMaxiWorker02.shares(1)),
              "expect Alice's balance is correct").to.eq(aliceBalance)
            expect(await cake.balanceOf(DEPLOYER), "expect DEPLOYER's CAKE is 0.001799999999997691 CAKE")
              .to.be.eq(ethers.utils.parseEther('0.001799999999997691'))
            expect(
              (await wbnb.balanceOf(integratedVault.address)).sub(ethers.utils.parseEther('1').sub(ethers.utils.parseEther('0.05'))),
              "expect WBNB balance in Beneficial Vault remain 0.004291917527507221 WBNB"
            ).to.be.eq(ethers.utils.parseEther('0.004291917527507221'))
            expect(await integratedCakeMaxiWorker02.buybackAmount(), "expect buyback amount is 0.002844237418144941")
    ...
    context("When integrated with an actual vault", async () => {
      context("when there is buybackAmount left when killing a position", async() => {
        it('should successfully liquidate a certain position without any returning a buybackAmount', async () => {
          ...
          await routerV2AsAlice.swapTokensForExactETH(ethers.utils.parseEther('0.935'), constants.MaxUint256, [cake.address, wbnb.address], await alice.getAddress(), FOREVER)
          ...
          const repaid = debt.gte(toBeLiquidatedValue.sub(totalLiquidationFees)) ? toBeLiquidatedValue.sub(totalLiquidationFees) : debt
          ...
          expect(
            ...
            "expect Bob to get a correct liquidation bounty"
          ).to.eq(liquidationBounty)
          expect(
            aliceBalanceAfter.sub(aliceBalanceBefore),
            "expect Alice to get a correct left amount"
          ).to.eq(left) 
          expect(
            vaultBalanceAfter.sub(vaultBalanceBefore),
            "expect Vault should get its funds back"
          ).to.eq(repaid)
          expect(
            deployerBalanceAfter.sub(deployerBalanceBefore),
            "expect Deployer should get tresaury fees"
          ).to.eq(treasuryKillFees)
  ...
  describe("#setBeneficialVaultBountyBps", async() => {
    ...
    context('When the _beneficialVaultBountyBps > 10000 (100%)', async() => {
      it('should be reverted', async() => {
        await expect(cakeMaxiWorkerNonNative.setBeneficialVaultBountyBps(BigNumber.from('10001'))).to.revertedWith('CakeMaxiWorker02::setBeneficialVaultBountyBps:: _beneficialVaultBountyBps exceeds 100%')
      })
    })
    ...
    context('when the param is correct', async () => {
      it('should successfully set the beneficial vault bounty bps', async () => {
        ...
        await expect(cakeMaxiWorkerNonNative.setBeneficialVaultBountyBps(BigNumber.from('10000'))).not.to.revertedWith('CakeMaxiWorker02::setBeneficialVaultBountyBps:: _beneficialVaultBountyBps exceeds 100%')
        expect(await cakeMaxiWorkerNonNative.beneficialVaultBountyBps()).to.eq(BigNumber.from('10000'))
        await expect(cakeMaxiWorkerNonNative.setBeneficialVaultBountyBps(BigNumber.from('5000'))).not.to.revertedWith('CakeMaxiWorker02::setBeneficialVaultBountyBps:: _beneficialVaultBountyBps exceeds 100%')
