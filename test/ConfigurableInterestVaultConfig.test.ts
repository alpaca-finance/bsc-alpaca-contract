...
describe('Configuration Vault ', () => {
  ...
  beforeEach(async () => {
    ...
    const ConfigurableInterestVaultConfig = (await ethers.getContractFactory("ConfigurableInterestVaultConfig",deployer,
    )) as ConfigurableInterestVaultConfig__factory
    configVault = await upgrades.deployProxy(ConfigurableInterestVaultConfig, [MIN_DEBT_SIZE,RESERVE_POOL_BPS,KILL_PRIZE_BPS,INTEREST_MODEL,
        WRAP_NATIVE_ADDR,WNATIVE_RELAYER,FAIR_LAUNCH_ADDR,KILL_TREASURY_BPS,TREASURY_ADDR]) as ConfigurableInterestVaultConfig
  ...
  context('when update Config Vault\'s params', async() => {
    it('should return treasury address correctly when update', async() => {
        ...
        await configVault.setParams(
            MIN_DEBT_SIZE,RESERVE_POOL_BPS,KILL_PRIZE_BPS,INTEREST_MODEL,
            WRAP_NATIVE_ADDR,WNATIVE_RELAYER,FAIR_LAUNCH_ADDR,KILL_TREASURY_BPS,deployerAddress
        )
    ...
    it('should reverted with over maxbps < kill_prize + kill_treasury', async() => {
        ...
        const treasuryOver = '600';
         ...
         configVault.setParams(
            MIN_DEBT_SIZE,RESERVE_POOL_BPS,killPrizeOver,INTEREST_MODEL,
            WRAP_NATIVE_ADDR,WNATIVE_RELAYER,FAIR_LAUNCH_ADDR,ethers.constants.Zero,deployerAddress
        )).to.be.revertedWith('ConfigurableInterestVaultConfig::setParams:: kill bps exceeded max kill bps')

        expect(
            configVault.setParams(
               MIN_DEBT_SIZE,RESERVE_POOL_BPS,ethers.constants.Zero,INTEREST_MODEL,
               WRAP_NATIVE_ADDR,WNATIVE_RELAYER,FAIR_LAUNCH_ADDR,treasuryOver,deployerAddress
           )).to.be.revertedWith('ConfigurableInterestVaultConfig::setParams:: kill bps exceeded max kill bps')
    })
  })
