...
describe('WaultSwapRestrictedStrategyWithdrawMinimizeTrading', () => {
  ...
  /// Strategy instance(s)
  let strat: WaultSwapRestrictedStrategyWithdrawMinimizeTrading;
  ...
  let stratAsAlice: WaultSwapRestrictedStrategyWithdrawMinimizeTrading;
  let stratAsBob: WaultSwapRestrictedStrategyWithdrawMinimizeTrading;
  ...
  beforeEach(async () => {
    ...
    strat = await upgrades.deployProxy(WaultSwapRestrictedStrategyWithdrawMinimizeTrading, [router.address, wbnb.address, wNativeRelayer.address]) as WaultSwapRestrictedStrategyWithdrawMinimizeTrading;
    ...
    stratAsAlice = WaultSwapRestrictedStrategyWithdrawMinimizeTrading__factory.connect(strat.address, alice);
    stratAsBob = WaultSwapRestrictedStrategyWithdrawMinimizeTrading__factory.connect(strat.address, bob);
  ...
  context('When the setOkWorkers caller is not an owner', async() => {
    it('should be reverted', async () => {
      await expect(stratAsBob.setWorkersOk([mockWaultSwapV2EvilWorkerAsBob.address], true)).to.reverted
    })
  })
  ...
  context("When caller worker hasn't been whitelisted", async () => {
    it('should revert as bad worker', async () => {
      await baseTokenAsBob.transfer(mockWaultSwapV2EvilWorkerAsBob.address, ethers.utils.parseEther('0.05'));
      await expect(mockWaultSwapV2EvilWorkerAsBob.work(
      ...
      )).to.be.revertedWith('WaultSwapRestrictedStrategyWithdrawMinimizeTrading::onlyWhitelistedWorkers:: bad worker');
    });
  })
  ...
  context('It should convert LP tokens and farming token', () => {
    ...
    it('should revert, Bob uses withdraw minimize trading strategy to turn LPs back to farming with an unreasonable expectation', async () => {
      ...
      )).to.be.revertedWith('WaultSwapRestrictedStrategyWithdrawMinimizeTrading::execute:: insufficient farming tokens received')
    });
  ...
  context('It should handle properly when the farming token is WBNB', () => {
    beforeEach(async () => {
      ...
      await baseTokenWbnbLpV2AsBob.transfer(mockWaultSwapBaseTokenWbnbV2Worker.address, ethers.utils.parseEther('0.316227766016837933'));
    });
    ...
    it('should convert all LP tokens back to BaseToken and BNB, while debt == received BaseToken', async () => {
      ...
      await mockWaultSwapBaseTokenWbnbV2WorkerAsBob.work(
        0, await bob.getAddress(), ethers.utils.parseEther('1'),
        ethers.utils.defaultAbiCoder.encode(
          ['address', 'bytes'],
          [strat.address, ethers.utils.defaultAbiCoder.encode(
    ...
    it('should revert when debt > received BaseToken, BNB is not enough to cover the debt', async () => {
        await expect(mockWaultSwapBaseTokenWbnbV2WorkerAsBob.work(
            0, await bob.getAddress(), ethers.utils.parseEther('3'),
            ethers.utils.defaultAbiCoder.encode(
                ['address', 'bytes'],
                [strat.address, ethers.utils.defaultAbiCoder.encode(
