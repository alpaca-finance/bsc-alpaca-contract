...
describe('WaultSwapRestrictedStrategyLiquidate', () => {
  ...
  beforeEach(async () => {
    ...
    strat = await upgrades.deployProxy(WaultSwapRestrictedStrategyLiquidate, [router.address]) as WaultSwapRestrictedStrategyLiquidate;
  ...
  context("When caller worker hasn't been whitelisted", async () => {
    it('should revert as bad worker', async () => {
      ...
      await expect(mockWaultSwapWorkerEvilAsBob.work(
  ...
  it('should convert all LP tokens back to baseToken', async () => {
    ...
    expect(await farmingToken.balanceOf(lp.address)).to.be.bignumber.eq(ethers.utils.parseEther('0.2'))
    assertAlmostEqual(ethers.utils.parseEther('100.5').toString(), (await baseToken.balanceOf(await bob.getAddress())).toString())
  });
});
