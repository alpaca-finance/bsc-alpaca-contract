  ...
  describe('#setPriceFeed', async () => {
    context('when the caller is not the owner', async () => {
      it('should be reverted', async () => {
        await expect(
          chainLinkOracleAsAlice.setPriceFeed(token0.address, token1.address, mockAggregatorV3T0T1.address),
    ...
    context('when the caller is the owner', async () => {
      context('when source on existed pair', async () => {
        it('should be reverted', async () => {
          await chainLinkOracleAsDeployer.setPriceFeed(token0.address, token1.address, mockAggregatorV3T0T1.address)
          await expect(
            chainLinkOracleAsDeployer.setPriceFeed(token1.address, token0.address, mockAggregatorV3T1T0.address),
      ...
      context('when successfully', async () => {
        it('should successfully', async () => {
          await expect(
            chainLinkOracleAsDeployer.setPriceFeed(token0.address, token1.address, mockAggregatorV3T0T1.address),
          ...
          expect(source).to.eq(mockAggregatorV3T0T1.address)
        })
      })
    })
  })
