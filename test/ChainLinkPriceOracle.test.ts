  ...
  describe('#setPriceFeeds', async () => {
    ...
    context('when the caller is the owner', async () => {
      context('when inconsistent length', async () => {
        it('should be reverted', async () => {
          await chainLinkOracleAsDeployer.setPriceFeeds([token0.address], [token1.address], [mockAggregatorV3T0T1.address])
          await expect(
            chainLinkOracleAsDeployer.setPriceFeeds([token1.address, token2.address], [token0.address], [mockAggregatorV3T1T0.address]),
          ).to.revertedWith('ChainLinkPriceOracle::setPriceFeeds:: inconsistent length')
          await expect(
            chainLinkOracleAsDeployer.setPriceFeeds([token1.address, token2.address], [token0.address, token3.address], [mockAggregatorV3T1T0.address]),
          ).to.revertedWith('ChainLinkPriceOracle::setPriceFeeds:: inconsistent length')
        })
      })
      ...
      context('when successfully', async () => {
        it('should successfully', async () => {
          await expect(
            chainLinkOracleAsDeployer.setPriceFeeds([token0.address, token2.address], [token1.address, token3.address], [mockAggregatorV3T0T1.address, mockAggregatorV3T2T3.address]),
