...
describe("Vault - WaultSwap", () => {
  ...
  async function fixture() {
    ...
    const WaultSwapToken = (await ethers.getContractFactory("WaultSwapToken", deployer)) as WaultSwapToken__factory;
    wex = await WaultSwapToken.deploy();
    await wex.deployed();
    await wex.mint(deployerAddress, ethers.utils.parseEther("100"));
    ...
    /// Setup wexMaster
    const WexMaster = (await ethers.getContractFactory("WexMaster", deployer)) as WexMaster__factory;
    wexMaster = await WexMaster.deploy(wex.address, WEX_REWARD_PER_BLOCK, 0);
    ...
    addStrat = (await upgrades.deployProxy(WaultSwapRestrictedStrategyAddBaseTokenOnly, [
      router.address,
    ...
    liqStrat = (await upgrades.deployProxy(WaultSwapRestrictedStrategyLiquidate, [
      router.address,
    ...
    partialCloseStrat = (await upgrades.deployProxy(WaultSwapRestrictedStrategyPartialCloseLiquidate, [
      router.address,
    ...
    partialCloseMinimizeStrat = (await upgrades.deployProxy(WaultSwapRestrictedStrategyPartialCloseMinimizeTrading, [
      router.address,
      wbnb.address,
      wNativeRelayer.address,
  ...
  context("when owner is setting worker", async () => {
    ...
    it("should revert when owner set reinvestBountyBps > max", async () => {
      await expect(waultSwapWorker.setReinvestBountyBps(1000)).to.be.revertedWith(
  ...
  context("when user uses LYF", async () => {
    ...
    context("#onlyApprovedHolders", async () => {
    ...
    context("#reinvest", async () => {
      it("should reinvest correctly", async () => {
        ...
        // LP tokens of worker should be inceased from reinvestment
        expect(workerLPAfter).to.be.bignumber.gt(workerLPBefore);
        AssertHelpers.assertAlmostEqual(workerLPAfter.sub(workerLPBefore).toString(), "13732120454748601");
      ...
      context("#liquidate", async () => {
        context("when maxReturn is lessDebt", async () => {
          ...
          it("should pay debt 'maxReturn' BTOKEN and return 'liquidatedAmount - maxReturn' BTOKEN to user", async () => {
                ...
                [
                  ...
                  ethers.utils.defaultAbiCoder.encode(
                    ...
                    [
                      ...
                      ethers.utils.parseEther("5"),
                      ethers.utils.parseEther("9.62878631929428108"),
                    ]
                  ),
                ]
              )
            ...
            // 14.628786319294281080 BTOKEN (price impact+trading fee included)
            // Bob returns 5 BTOKEN to payback the debt hence; He should be
            // 14.628786319294281080 - 5 = 9.62878631929428108 BTOKEN richer
            expect(bobAfter.sub(bobBefore), "Bob should get 9.62878631929428108 BTOKEN back").to.be.bignumber.eq(
              ethers.utils.parseEther("9.62878631929428108")
            );
            // Check Bob position info
            [bobHealth, bobDebtToShare] = await vault.positionInfo("1");
        ...
        context("when debt is lessDebt", async () => {
          ...
          it("should pay back all debt and return 'liquidatedAmount - debt' BTOKEN to user", async () => {
                ...
                [
                  ...
                  ethers.utils.defaultAbiCoder.encode(
                    ...
                    [
                      lpUnderBobPosition.div(2),
                      ethers.utils.parseEther("5000000000"),
                      ethers.utils.parseEther("3.200430549066301204"),
                    ]
                  ),
                ]
              )
            ...
            expect(
              ...
              "Expect BTOKEN in Bob's account after close position to increase by ~3.20 BTOKEN"
