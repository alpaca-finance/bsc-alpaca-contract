...
import {
  ...
  MockWBNB,
  MockWBNB__factory,
  InvestinStrategyToken,
  InvestinStrategyToken__factory,
  PancakeFactory,
...
describe("Vault - PancakeswapV2", () => {
  ...
  let ivnStrategyToken: InvestinStrategyToken;
  let IVNy: IVNyToken;
  ...
  beforeEach(async () => {
    ...
    // Investin strartegy creation
    const IVNY = (await ethers.getContractFactory("IVNyToken",deployer)) as IVNyToken__factory; 
    IVNy = await IVNY.deploy(deployerAddress) as IVNyToken;
    await IVNy.deployed();
    const IvnStrategyToken = (await ethers.getContractFactory("InvestinStrategyToken", deployer)) as InvestinStrategyToken__factory;
    ivnStrategyToken  = await IvnStrategyToken.deploy(vault.address,baseToken.address, IVNy.address)  as InvestinStrategyToken;
    await ivnStrategyToken.deployed();
  ...
  context("when user uses LYF", async () => {
    ...
    context("when user is EOA", async () => {
      context("#work", async () => {
        ...
        it("should allow to open a position without debt after Investin strategy entry", async () => {
          // Deployer deposits 3 BTOKEN to the bank
          await baseToken.approve(vault.address, ethers.utils.parseEther("3"));
          await vault.deposit(ethers.utils.parseEther("3"));

          // Check for Price match from ibToken Vault and Investin Strategy
          let totalToken : BigNumber = await vault.totalToken();
          let totalSupply : BigNumber = await vault.totalSupply();
          let priceOfVaultToken : BigNumber = (totalToken.mul("1000000000000000000").div(totalSupply));
          let priceOfInvestinStrategyToken : BigNumber = await ivnStrategyToken.getStrategyTokenPrice();
          AssertHelpers.assertAlmostEqual(
                priceOfVaultToken.toString(),
                priceOfInvestinStrategyToken.toString()
          );

          //Deployer deposits 3 BTOKEN to the bank through Investin strat
          const enterAmount: BigNumber = ethers.utils.parseEther("3");
          await baseToken.approve(ivnStrategyToken.address, enterAmount);
          let amountOutMin : BigNumber = (enterAmount.mul("989010000000000000").div(priceOfVaultToken));
          await  ivnStrategyToken["enter(uint256,bool,uint256)"](enterAmount, false, amountOutMin);
          let stbalance: BigNumber = await ivnStrategyToken.balanceOf(deployerAddress);
            
          AssertHelpers.assertAlmostEqual(
                enterAmount.mul("999").div("1000").toString(),
                stbalance.mul(priceOfInvestinStrategyToken).div("1000000000000000000").toString()
          );
        ...
        it("should work", async () => {
          // Deployer deposits 3 BTOKEN to the bank
          let deposit = ethers.utils.parseEther("3");
          ...
          //Deployer deposits 3 BTOKEN to the bank through Investin strat
          deposit = ethers.utils.parseEther("5.997");
          await baseToken.approve(ivnStrategyToken.address, ethers.utils.parseEther("3"));
          await ivnStrategyToken["enter(uint256,bool,uint256)"](ethers.utils.parseEther("3"),false, 0);
          ...
          // Check for Price match from ibToken Vault and Investin Strategy after reinvest
          totalToken  = await vault.totalToken();
          totalSupply  = await vault.totalSupply();
          priceOfVaultToken  = (totalToken.mul("1000000000000000000").div(totalSupply));
          priceOfInvestinStrategyToken = await ivnStrategyToken.getStrategyTokenPrice();
            AssertHelpers.assertAlmostEqual(
                priceOfVaultToken.toString(),
                priceOfInvestinStrategyToken.toString()
            );
          ...
          // Check for Price match from ibToken Vault and Investin Strategy after interest computation
          totalToken  = await vault.totalToken();
          totalSupply  = await vault.totalSupply();
          priceOfVaultToken  = (totalToken.mul("1000000000000000000").div(totalSupply));
          priceOfInvestinStrategyToken = await ivnStrategyToken.getStrategyTokenPrice();
        ...
        it("should has correct interest rate growth", async () => {
          ...
          deposit = ethers.utils.parseEther("5.997");
          await baseToken.approve(ivnStrategyToken.address, ethers.utils.parseEther("3"));
          await ivnStrategyToken["enter(uint256,bool,uint256)"](ethers.utils.parseEther("3"), false, ethers.utils.parseEther("2"));
        ...
        it("should close position correctly when user holds multiple positions", async () => {
          ...
          // Check for Price match from ibToken Vault and Investin Strategy
          let totalToken : string = ethers.utils.formatEther(await vault.totalToken());
          let totalSupply : string= ethers.utils.formatEther(await vault.totalSupply())
          let priceOfVaultToken : any = (parseFloat(totalToken)/parseFloat(totalSupply)).toString();
          let priceOfInvestinStrategyToken : any = parseFloat(ethers.utils.formatEther(await ivnStrategyToken.getStrategyTokenPrice())).toString();
          expect(priceOfVaultToken , "Price of ibToken doesn't match price from strategy").to.be.eq(priceOfInvestinStrategyToken);
          ...
          // Check for Price match from ibToken Vault and Investin Strategy after
          totalToken = ethers.utils.formatEther(await vault.totalToken());
          totalSupply = ethers.utils.formatEther(await vault.totalSupply())
          priceOfVaultToken = (parseFloat(totalToken)/parseFloat(totalSupply)).toString();
          priceOfInvestinStrategyToken = parseFloat(ethers.utils.formatEther(await ivnStrategyToken.getStrategyTokenPrice())).toString();
          expect(priceOfVaultToken , "Price of ibToken doesn't match price from strategy").to.be.eq(priceOfInvestinStrategyToken);
      ...
      context("#deposit-withdraw", async () => {
        it("should deposit and withdraw BTOKEN from Vault thorugh Investin Strategy (bad debt case)", async () => {
          ...
          // Alice deposits 2 BTOKEN Through Investin Strategy
          const aliceDeposit: BigNumber = ethers.utils.parseEther("2");
          await baseTokenAsAlice.approve(ivnStrategyToken.address, ethers.utils.parseEther("2"));
          
          let amountOutMin : BigNumber = (aliceDeposit.mul("989010000000000000").div(priceOfVaultToken));
          await ivnStrategyToken.connect(alice)["enter(uint256,bool,uint256)"](aliceDeposit, false, amountOutMin);
          let stbalance: BigNumber = await ivnStrategyToken.balanceOf(aliceAddress);
            
          AssertHelpers.assertAlmostEqual(
                aliceDeposit.mul("999").div("1000").toString(),
                stbalance.mul(priceOfInvestinStrategyToken).div("1000000000000000000").toString()
          );

          AssertHelpers.assertAlmostEqual(
            deposit.sub(loan).add(aliceDeposit.mul("999").div("1000")).toString(),
            (await baseToken.balanceOf(vault.address)).toString()
          );

          

          // check Alice ibBTOKEN balance = 2(1-0.999)/10 * 10 = 1.998 ibBTOKEN
          AssertHelpers.assertAlmostEqual(aliceDeposit.mul("999").div("1000").toString(), (await ivnStrategyToken.balanceOf(aliceAddress)).toString());
          AssertHelpers.assertAlmostEqual(deposit.add(stbalance).toString(), (await vault.totalSupply()).toString());
          ...
          AssertHelpers.assertAlmostEqual(
            ethers.utils.parseEther("10.000671778489331742").toString(),
            (await baseToken.balanceOf(vault.address)).toString()
          );
          ...
          // Check for Price match from ibToken Vault and Investin Strategy after Liquidation
          totalToken  = await vault.totalToken();
          totalSupply  = await vault.totalSupply();
          priceOfVaultToken  = (totalToken.mul("1000000000000000000").div(totalSupply));
          priceOfInvestinStrategyToken = await ivnStrategyToken.getStrategyTokenPrice();
          ...
          // Alice withdraws 1.998 Strategy Tokens
          aliceBefore = await baseToken.balanceOf(aliceAddress);
          amountOutMin = (stbalance.mul(priceOfVaultToken).div("100000000000000000000"));
          await ivnStrategyToken.connect(alice)["exit(uint256,uint256)"](stbalance, amountOutMin);
          aliceAfter = await baseToken.balanceOf(aliceAddress);

          // alice gots 1.998/11.998 * 10.000671778489331742 = 1.66538941602
          AssertHelpers.assertAlmostEqual(
            ethers.utils.parseEther("1.66538941602").toString(),
            aliceAfter.sub(aliceBefore).toString()
          );
        });
      });
        ...
        context("#minimize", async () => {
          context("when maxReturn is lessDebt", async () => {
            ...
            it("should pay debt 'maxReturn' BTOKEN and return 'liquidatedAmount - maxReturn' BTOKEN to user ", async () => {
              ...
              ).to.be.bignumber.eq(bobFTokenAfter);
              // Check Bob position info
              const [bobHealth, bobDebtVal] = await vault.positionInfo("1");
              // Bob's health after partial close position must be 50% less than before
              // due to he exit half of lp under his position
