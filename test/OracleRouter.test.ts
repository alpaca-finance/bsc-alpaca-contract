...
import { MockERC20, MockERC20__factory, OracleRouter, OracleRouter__factory, SimplePriceOracle, SimplePriceOracle__factory } from "../typechain"
...
let simplePriceOracleAsDeployer: SimplePriceOracle
...
describe('OracleRouter', () => {
    beforeEach(async () => {
        ...
        simplePriceOracle = await upgrades.deployProxy(SimplePriceOracle, [await feeder.getAddress()]) as SimplePriceOracle
        await simplePriceOracle.deployed()
        simplePriceOracleAsDeployer = SimplePriceOracle__factory.connect(simplePriceOracle.address, deployer)
    ...
    describe('#setPrimarySources', async () => {
        context('when the caller is not the owner', async () => {
            it('should be reverted', async () => {
                await expect(oracleRouterAsAlice.setPrimarySources(token0.address, token1.address, BigNumber.from('1000000000000000000'), [simplePriceOracle.address])).to.revertedWith("Ownable: caller is not the owner")
            })
        })
        context('when the caller is the owner', async () => {
            context('when bad max deviation value', async () => {
                it('should be reverted', async () => {
                    await expect(oracleRouterAsDeployer.setPrimarySources(token0.address, token1.address, BigNumber.from('0'), [simplePriceOracle.address])).to.revertedWith("bad max deviation value")
                })
                it('should be reverted', async () => {
                    await expect(oracleRouterAsDeployer.setPrimarySources(token0.address, token1.address, BigNumber.from('2000000000000000000'), [simplePriceOracle.address])).to.revertedWith("bad max deviation value")
                })
            })
            context('when sources length exceed 3', async () => {
                it('should be reverted', async () => {
                    await expect(oracleRouterAsDeployer.setPrimarySources(token0.address, token1.address, BigNumber.from('1000000000000000000'), [simplePriceOracle.address, simplePriceOracle.address, simplePriceOracle.address, simplePriceOracle.address])).to.revertedWith("sources length exceed 3")
                })
            })
            context('when successfully', async () => {
                it('should successfully', async () => {
                    await oracleRouterAsDeployer.setPrimarySources(token0.address, token1.address, BigNumber.from('1000000000000000000'), [simplePriceOracle.address])
    ...
    describe('#setMultiPrimarySources', async () => {
        context('when inconsistent length', async () => {
            it('should be reverted', async () => {
                await expect(oracleRouterAsDeployer.setMultiPrimarySources([token0.address, token2.address], [token1.address], [BigNumber.from('1000000000000000000')], [[simplePriceOracle.address]])).to.revertedWith("inconsistent length")
            })
            it('should be reverted', async () => {
                await expect(oracleRouterAsDeployer.setMultiPrimarySources([token0.address, token2.address], [token1.address, token3.address], [BigNumber.from('1000000000000000000')], [[simplePriceOracle.address]])).to.revertedWith("inconsistent length")
            })
            it('should be reverted', async () => {
                await expect(oracleRouterAsDeployer.setMultiPrimarySources([token0.address, token2.address], [token1.address, token3.address], [BigNumber.from('1000000000000000000'), BigNumber.from('900000000000000000')], [[simplePriceOracle.address]])).to.revertedWith("inconsistent length")
            })
        })
        context('when successfully', async () => {
            it('should successfully', async () => {
                await oracleRouterAsDeployer.setMultiPrimarySources([token0.address, token2.address], [token1.address, token3.address], [BigNumber.from('1000000000000000000'), BigNumber.from('1100000000000000000')], [[simplePriceOracle.address],[simplePriceOracle.address, bobPriceOracle.address]])
    ...
    describe('#getPrice', async () => {
        ...
        context('when has 1 valid sources', async () => {
            it('should successfully', async () => {
                await simplePriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('1000000000000000000'), BigNumber.from('1000000000000000000').div(10)])
                await oracleRouterAsDeployer.setPrimarySources(token0.address, token1.address, BigNumber.from('1000000000000000000'), [simplePriceOracle.address])
        ...
        context('when has 2 valid sources', async () => {
            context('when too much deviation (2 valid sources)', async () => {
                it('should be reverted', async () => {
                    await simplePriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('1000000000000000000'), BigNumber.from('1000000000000000000').div(10)])
                    await bobPriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('900000000000000000'), BigNumber.from('1000000000000000000').div(9)])

                    await oracleRouterAsDeployer.setPrimarySources(token0.address, token1.address, BigNumber.from('1100000000000000000'), [simplePriceOracle.address, bobPriceOracle.address])
            ...
            context('when successfully', async () => {
                it('should successfully', async () => {
                    ...
                    await oracleRouterAsDeployer.setPrimarySources(token0.address, token1.address, BigNumber.from('1200000000000000000'), [simplePriceOracle.address, bobPriceOracle.address])
                    const [price, lastTime] = await oracleRouterAsAlice.getPrice(token0.address, token1.address)
        ...
        context('when has 3 valid sources', async () => {
            context('when too much deviation', async () => {
                it('should be reverted', async () => {
                    await simplePriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('1000000000000000000'), BigNumber.from('1000000000000000000').div(10)])
                    await bobPriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('900000000000000000'), BigNumber.from('1000000000000000000').div(9)])
                    await evePriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('800000000000000000'), BigNumber.from('1000000000000000000').div(8)])

                    await oracleRouterAsDeployer.setPrimarySources(token0.address, token1.address, BigNumber.from('1100000000000000000'), [simplePriceOracle.address, bobPriceOracle.address, evePriceOracleAsFeeder.address])
                    await expect(oracleRouterAsAlice.getPrice(token0.address, token1.address)).to.reverted
                })
            })
            context('when source1 and source2 has too much deviation', async () => {
                it('should be successfully', async () => {
                    await simplePriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('1100000000000000000'), BigNumber.from('1000000000000000000').div(11)])
                    await bobPriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('900000000000000000'), BigNumber.from('1000000000000000000').div(9)])
                    await evePriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('800000000000000000'), BigNumber.from('1000000000000000000').div(8)])

                    await oracleRouterAsDeployer.setPrimarySources(token0.address, token1.address, BigNumber.from('1200000000000000000'), [simplePriceOracle.address, bobPriceOracle.address, evePriceOracleAsFeeder.address])
                    const [price, lastTime] = await oracleRouterAsAlice.getPrice(token0.address, token1.address)
            ...
            context('when source2 and source3 has too much deviation', async () => {
                it('should be successfully', async () => {
                    await simplePriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('1000000000000000000'), BigNumber.from('1000000000000000000').div(10)])
                    await bobPriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('900000000000000000'), BigNumber.from('1000000000000000000').div(9)])
                    await evePriceOracleAsFeeder.setPrices([token0.address, token1.address], [token1.address, token0.address], [BigNumber.from('700000000000000000'), BigNumber.from('1000000000000000000').div(7)])
