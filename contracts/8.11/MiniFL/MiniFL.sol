...
contract MiniFL is IMiniFL, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function addPool(
    ...
    if (isStakingToken[address(_stakingToken)] == true) revert MiniFL_DuplicatePool();
  ...
  function setAlpacaPerSecond(uint256 _alpacaPerSecond) public onlyOwner {
    massUpdatePools();
  ...
  function pendingAlpaca(uint256 _pid, address _user) external view returns (uint256) {
    ...
    return (int256((user.amount * accAlpacaPerShare) / ACC_ALPACA_PRECISION) - user.rewardDebt).toUint256();
  }
  ...
  function deposit(
    ...
    if (pool.isDebtTokenPool == true && stakeDebtTokenAllowance[_pid][msg.sender] == false) revert MiniFL_Forbidden();
    if (pool.isDebtTokenPool == false && msg.sender != _for) revert MiniFL_Forbidden();

    // Effects
    user.amount = user.amount + _amount;
    user.rewardDebt = user.rewardDebt + (int256((_amount * pool.accAlpacaPerShare) / ACC_ALPACA_PRECISION));
  ...
  function harvest(uint256 _pid) public {
    ...
    int256 accumulatedAlpaca = int256((user.amount * pool.accAlpacaPerShare) / ACC_ALPACA_PRECISION);
