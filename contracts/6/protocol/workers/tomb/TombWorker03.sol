...
/// @title TombWorker03 is a worker with reinvest-optimized and beneficial vault buyback functionalities
// solhint-disable not-rely-on-time
// solhint-disable avoid-tx-origin
// solhint-disable max-states-count
contract TombWorker03 is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker03 {
  /// @notice Libraries
  ...
  function initialize(
    address _operator,
    address _baseToken,
    ITShareRewardPool _tshareRewardPool,
    ...
    tshareRewardPool = _tshareRewardPool;
    router = _router;
    ...
    require(baseToken != tshare, "baseToken must !tshare");
    ...
    require(reinvestPath[0] == tshare && reinvestPath[reinvestPath.length - 1] == baseToken, "bad reinvest path");
  }
  ...
  function shareToBalance(uint256 share) public view returns (uint256) {
    ...
    (uint256 totalBalance, ) = tshareRewardPool.userInfo(pid, address(this));
  ...
  function _reinvest(
    ...
    tshareRewardPool.withdraw(pid, 0);
    uint256 reward = tshare.balanceOf(address(this));
    if (reward <= _reinvestThreshold) return;

    // 2. Approve tokens
    tshare.safeApprove(address(router), uint256(-1));
    address(lpToken).safeApprove(address(tshareRewardPool), uint256(-1));
    ...
    if (bounty > 0) {
      ...
      tshare.safeTransfer(_treasuryAccount, bounty.sub(beneficialVaultBounty));
    }
    ...
    tshareRewardPool.deposit(pid, lpToken.balanceOf(address(this)));

    // 7. Reset approvals
    tshare.safeApprove(address(router), 0);
    address(lpToken).safeApprove(address(tshareRewardPool), 0);
  ...
  function _addShare(uint256 id) internal {
    ...
    if (balance > 0) {
      // 1. Approve token to be spend by TShareRewardPool
      address(lpToken).safeApprove(address(tshareRewardPool), uint256(-1));
      // 2. Convert balance to share
      uint256 share = balanceToShare(balance);
      require(share > 0, "no zero share");
      // 3. Deposit balance to TShareRewardPool
      tshareRewardPool.deposit(pid, balance);
      // 4. Update shares
  ...
  function _removeShare(uint256 id) internal {
    ...
    if (share > 0) {
      ...
      tshareRewardPool.withdraw(pid, balance);
  ...
  function setReinvestConfig(
    ...
    require(_reinvestPath[0] == tshare && _reinvestPath[_reinvestPath.length - 1] == baseToken, "bad _reinvestPath");
  ...
  function setRewardPath(address[] calldata _rewardPath) external onlyOwner {
    ...
    require(
      _rewardPath[0] == tshare && _rewardPath[_rewardPath.length - 1] == beneficialVault.token(),
      "bad _rewardPath"
    );
  ...
  function setBeneficialVaultConfig(
    ...
    require(
      _rewardPath[0] == tshare && _rewardPath[_rewardPath.length - 1] == _beneficialVault.token(),
      "bad _rewardPath"
    );
