...
import {
  ...
  PancakeswapV2Worker__factory, Vault, Vault__factory, WaultSwapRestrictedStrategyAddBaseTokenOnly__factory, WaultSwapRestrictedStrategyAddTwoSidesOptimal__factory, WaultSwapRestrictedStrategyLiquidate__factory, WaultSwapRestrictedStrategyWithdrawMinimizeTrading__factory, WaultSwapWorker, WaultSwapWorker__factory,
} from "../typechain";
...
async function validateWorker(vault: Vault, workerInfo: IWorkerInfo, routers: IDexRouter) {
...
async function main() {
  ...
  try {
    ...
    const pcsBaseTokenOnlyStrat = PancakeswapV2RestrictedStrategyAddBaseTokenOnly__factory.connect(config.SharedStrategies.Pancakeswap.StrategyAddBaseTokenOnly, ethers.provider)
    expect(await pcsBaseTokenOnlyStrat.router()).to.be.eq(config.Exchanges.Pancakeswap.RouterV2, "pcs add base strat router mis-config")

    const pcsLiqStrat = PancakeswapV2RestrictedStrategyLiquidate__factory.connect(config.SharedStrategies.Pancakeswap.StrategyLiquidate, ethers.provider)
    expect(await pcsLiqStrat.router()).to.be.eq(config.Exchanges.Pancakeswap.RouterV2, "pcs liq strat router mis-config")

    const pcsMinimizeTrade = PancakeswapV2RestrictedStrategyWithdrawMinimizeTrading__factory.connect(config.SharedStrategies.Pancakeswap.StrategyWithdrawMinimizeTrading, ethers.provider)
    expect(await pcsMinimizeTrade.router()).to.be.eq(config.Exchanges.Pancakeswap.RouterV2, "pcs minimize router mis-config")
    console.log("> ✅ done")

    console.log(`> validating Waultswap shared strats`)
    const wswapBaseTokenOnlyStrat = WaultSwapRestrictedStrategyAddBaseTokenOnly__factory.connect(config.SharedStrategies.Waultswap.StrategyAddBaseTokenOnly, ethers.provider)
    expect(await wswapBaseTokenOnlyStrat.router()).to.be.eq(config.Exchanges.Waultswap.WaultswapRouter)

    const wswapLiqStrat = WaultSwapRestrictedStrategyLiquidate__factory.connect(config.SharedStrategies.Waultswap.StrategyLiquidate, ethers.provider)
    expect(await wswapLiqStrat.router()).to.be.eq(config.Exchanges.Waultswap.WaultswapRouter)

    const wswapMinimizeTrade = WaultSwapRestrictedStrategyWithdrawMinimizeTrading__factory.connect(config.SharedStrategies.Waultswap.StrategyWithdrawMinimizeTrading, ethers.provider)
  ...
  for(let i = 0; i < config.Vaults.length; i++) {
    ...
    try {
      ...
      expect(await pcsTwoSides.router()).to.be.eq(config.Exchanges.Pancakeswap.RouterV2, "pcs twosides router mis-config")
